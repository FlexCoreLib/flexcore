...
template <class T>
auto port_description(const std::string& fallback, bool named) -> std::enable_if_t<has_token_type<T>(0), std::string>
{
	if (named)
		return "'" + fallback + "'";
...
struct graph_connectable : base_t
{
	...
	template <class... base_args>
	graph_connectable(const graph_node_properties& graph_info, bool named,
					  base_args&&... args)
		: base_t(std::forward<base_args>(args)...), graph_info(graph_info),
